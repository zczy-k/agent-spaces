...
| 时间 | 操作 | 说明 |
|------|------|------|
| 2026-06-03T23:00:29+08:00 | 增量更新 | **Workflow 编辑器大扩展**（10->28 组件：新增 ）；**新增 useWorkflowEditorStore**（768行完整编辑器状态管理，含撤销/重做/暂存/属性面板/版本/执行状态）；**新增 lib/workflow-api.ts + workflow-nodes.ts**（API 请求层 + 节点注册表）；**新增 hooks/use-workflow-editor.ts**（编辑器 Hook，桥接 store 和 ReactFlow）；**新增 stores/search-commands/workflow-search.ts**（Command Palette Workflow 搜索）；**i18n 命名空间 31 不变** |
| 2026-06-02T09:07:04+08:00 | 增量更新 | **六运行时支持**（runtimeKind 新增 oh-my-pi）；**Store 18->24**（新增 useDatabaseStore + useKanbanStore + useWorktreeStore + useEditorSendStore + useActivityLogStore + useContentUsageReport）；**Database 组件目录**（15 文件：notion-editor/markdown-editor/database-panel/database-sidebar/database-tree-node/database-vector-dialog/quick-search-modal/trash-bin-modal/version-history-dialog/database-ai-chat 等）；**Kanban 组件目录**（6 文件：kanban-board/kanban-column/kanban-card/column-modal/column-manage-dialog/task-modal）；**Worktree 组件目录**（3 文件：worktree-panel/worktree-card/create-worktree-dialog）；**Git 组件扩展**（9->20 文件：新增 git-settings-form/git-commit-dialog/git-panel-layout/git-commits-panel/git-commits-section/git-commit-log-list/git-changes-panel/commit-diff-viewer/git-op-log-dialog/git-not-initialized/git-remote-dialog）；**ForgeUI 组件**（animated-tabs）；**Sidebar 扩展**（新增 tools-dialog + layout-manager-dialog，32 文件）；**Lib 新增**（agent-store/agent-members/theme-style/themes/layout-templates/converter/terminal-registry/commands/users/monaco-loader/github/sample-logs，12->24 文件）；**i18n 命名空间 25+->31**（新增 commandPalette/composer/folderPicker/models/providers/task/terminal/workspace/workspaces/home/settings/tools）；**Theme Style System**（自定义主题预设 Mira + 自定义 CSS，localStorage 持久化）；**Layout Templates**（FlexLayout 布局模板保存/加载/管理）；**Agent Store**（在线模板导入/同步）；**Send to Issue/Channel**（Monaco 右键发送代码片段）；**Content Usage Report**（跨 Store/工作空间/终端统计）；**Terminal Registry**（xterm 实例注册表）；**Tools 设置页**（/settings/tools）；**Diff/JSON/Log Viewer**；**Commit Graph/Activity Graph**；**Settings 对话框拆分**（Appearance/Language/Account/Security/Git/Speech/RobotAccounts + Shortcuts + About + Custom Font）；**文件数 265->250（精确源文件计数）** |
