...
Next.js 16 前端应用，提供多 Agent 协同编程平台的用户界面。包含登录认证、工作空间管理、用量统计仪表盘、订阅余额面板、项目设置面板（通知配置+Prompt配置）、服务器切换器、文件夹浏览器、FlexLayout IDE 布局、Monaco 代码编辑器（TypeScript LSP 定义跳转/引用/诊断 + Model 缓存预加载 + 搜索面板 + 代码收藏 + Monaco Action Registry + 菜单栏 + 移动端适配）、xterm.js 终端（快捷命令+虚拟键盘）、TipTap 富文本聊天（含 @mention Agent + 语音识别输入 + 回复 AI 消息工作流）、结构化 AI 消息渲染（chain/tool-step/context-panel/context-usage/tool-detail/diff/token-usage/confirmation/subagent/ask-user_question）、Workflow 可视化编辑器（@xyflow/react DAG + @dagrejs/dagre 自动布局）、议题管理（含 Workflow 选择）、Git 操作面板（含设置表单 + commit diff viewer）、Agent 配置管理、LLM 模型管理、头像上传、移动端适配、i18n 中英文切换（next-intl，52 个组件已改造）、Native 通知（Tauri/Browser）、Command Palette（Ctrl+K 快捷面板，cmdk）、Iframe Tab 管理器、浮动面板/浮球、DOM Inspector 源码定位、Inspector 历史记录、独立设置页（Agents/Skills/MCPs/Models/Providers/Prompts）、通知中心对话框、Prompt 模板管理等核心功能。通过 Zustand 管理全局状态（16 个 Store），WebSocket 实现实时数据同步，支持多后端服务器切换。
...
| `/settings/prompts` | `src/app/settings/prompts/page.tsx` | Prompt 模板管理设置页（**新**） |
...
Monaco 编辑器集成 TypeScript LSP，提供定义跳转、引用、诊断等语义能力（**新**）。
...
| `components/editor/code-favorites-panel.tsx` | 代码收藏面板（列表/跳转/删除）（**新**） |
| `components/editor/add-favorite-dialog.tsx` | 添加代码收藏对话框（**新**） |
| `components/editor/code-editor.tsx` | Monaco Editor 集成 |
| `components/editor/code-editor-menu-bar.tsx` | 编辑器菜单栏（**新**） |
| `components/editor/code-editor-navigation.ts` | 编辑器导航（**新**） |
| `components/editor/code-editor-utils.ts` | 编辑器工具函数（**新**） |
| `components/editor/code-editor-clipboard.ts` | 编辑器剪贴板操作（**新**） |
| `components/editor/code-editor-mobile.ts` | 移动端编辑器适配（**新**） |
| `components/editor/code-editor-mobile-overlay.tsx` | 移动端只读覆盖层（**新**） |

### DOM Inspector 集成

基于 dom-inspector-hook 的元素源码定位功能（**新**）。
...
| 组件/文件 | 说明 |
|-----------|------|
| `components/chat/message-tool-step.tsx` | 工具调用步骤渲染（文件编辑/搜索/Webhook 等 16 种工具类型） |
...
| `components/composer/create-file-search-extension.ts` | 文件搜索 TipTap 扩展（**新**） |
...
| `monaco-languageclient` (10.7) | Monaco LSP 语言客户端（**新**） |
| `vscode-languageclient` (9.0.1) | VSCode Language Client 协议（**新**） |
| `vscode-ws-jsonrpc` (3.5.0) | LSP WebSocket 通信（**新**） |
...
| `copy-code` | 代码复制（**新**） |
| `tw-animate-css` | Tailwind 动画 CSS（**新**） |
...
| `useCodeFavoritesStore` | `stores/code-favorites.ts` | favorites, pendingFavorite | 代码收藏（CRUD + 待添加）（**新**） |
| `useInspectorHistoryStore` | `stores/inspector-history.ts` | histories | Inspector 历史记录（localStorage）（**新**） |
...
`src/lib/monaco-language-client.ts` 提供 TypeScript LSP 集成（**新**）：
...
`src/lib/monaco-action-registry.ts` 提供编辑器 Action 注册机制（**新**）：
...
| 时间 | 操作 | 说明 |
|------|------|------|
| 2026-05-19T09:45:03+08:00 | 增量更新 | **TypeScript LSP**（新增 lib/monaco-language-client.ts + lib/monaco-action-registry.ts + lib/monaco-builtin-actions.ts，monaco-languageclient 连接后端 /ws/lsp/typescript）；**代码收藏**（新增 stores/code-favorites.ts + components/editor/code-favorites-panel.tsx + add-favorite-dialog.tsx，Monaco 右键 Action + 面板查看/跳转/删除）；**Prompt 模板**（新增 app/settings/prompts/page.tsx + components/sidebar/prompts-dialog.tsx，CRUD + 批量应用）；**DOM Inspector**（新增 components/dev-inspector.tsx + stores/inspector-history.ts，inspector.jump 事件处理 + localStorage 历史记录）；**编辑器增强**（新增 code-editor-clipboard/navigation/utils/menu-bar/mobile/mobile-overlay 组件，编辑器 1->7 文件）；**聊天拆分**（chat-input 拆分为 chat-input-utils/attachments/agent-bar/info-bar 4 文件，新增 message-tool-step/message-context-panel/message-context-usage + create-file-search-extension）；**浮动组件**（新增 common/floating-panel.tsx + floating-ball.tsx）；**设置增强**（新增 components/settings/notification-settings-tab.tsx + workspace-prompt-section.tsx + workspace-info-section.tsx）；**Git 增强**（新增 components/git/commit-diff-viewer.tsx）；**新增依赖** monaco-languageclient + vscode-languageclient + vscode-ws-jsonrpc + @dnd-kit/core + @dnd-kit/sortable + @dnd-kit/utilities + copy-code + tw-animate-css；**文件数 215->245** |
| 2026-05-16T17:36:40+08:00 | 增量更新 | **Command Palette**（新增 stores/command-palette.ts + components/command-palette.tsx + components/ui/command.tsx + stores/search-commands/ 6 文件）；**订阅面板** + **语音识别** + **快捷命令** + **搜索面板** + **Iframe 管理** + **应用内通知** + **Skill/MCP 管理** + **独立设置页** + **UI/导航增强**；**文件数 168->215** |
