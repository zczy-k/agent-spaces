...
export const useDatabaseStore = create<DatabaseState & DatabaseActions>((set, get) => ({
  ...
  load: async (workspaceId) => {
    set({ loading: true });
    try {
      const nodes = await api<DocNode[]>(workspaceId, '');
  ...
  createNode: async (workspaceId, parentId, type) => {
    ...
    const node = await api<DocNode>(workspaceId, '', {
      method: 'POST',
      body: JSON.stringify({
  ...
  trashNode: async (workspaceId, nodeId) => {
    ...
    await api(workspaceId, `/${nodeId}/trash`, { method: 'PUT' });
  ...
  restoreNode: async (workspaceId, nodeId) => {
    await api(workspaceId, `/${nodeId}/restore`, { method: 'PUT' });
    const { nodes } = get();
  ...
  deleteNode: async (workspaceId, nodeId) => {
    ...
    await api(workspaceId, `/${nodeId}`, { method: 'DELETE' });
  ...
  moveNode: async (workspaceId, nodeId, parentId) => {
    ...
    await api(workspaceId, `/${nodeId}/move`, { method: 'PUT', body: JSON.stringify({ parentId }) });
