...
export default function MarkdownEditor({ contentMarkdown, onChange, theme = 'sans' }: MarkdownEditorProps) {
  ...
  const btn = (onClick: () => void, title: string, icon: React.ReactNode) => (
    <button type="button" onClick={onClick} className="p-1.5 rounded hover:bg-[#18181B] text-zinc-400 hover:text-zinc-200 transition-colors cursor-pointer" title={title}>{icon}</button>
  );

  const previewHtml = markdownToHtml(contentMarkdown);

  return (
    <div className="w-full flex flex-col h-[550px] lg:h-[650px] bg-[#09090B] border border-[#27272A] rounded-xl overflow-hidden">
      <div className="bg-[#09090B] border-b border-[#27272A] px-3 py-2 flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
        ...
        <div className="flex items-center gap-1 bg-[#18181B] p-1 rounded-lg shrink-0">
          {(['edit', 'split', 'preview'] as const).map(m => (
            <button key={m} type="button" onClick={() => setViewMode(m)}
              className={cn("flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-md transition-colors cursor-pointer",
                viewMode === m ? "bg-[#27272A] text-white" : "text-zinc-400 hover:text-zinc-200")}>
      ...
      <div className="flex-1 flex w-full min-h-0">
        {(viewMode === 'edit' || viewMode === 'split') && (
          <div className={cn("h-full flex flex-col min-w-0 flex-1 relative border-r border-[#27272A]", viewMode === 'edit' ? "w-full" : "w-1/2")}>
            <textarea ref={textareaRef} value={contentMarkdown} onChange={(e) => onChange(e.target.value)} onScroll={handleScroll}
              className={cn("w-full h-full p-6 text-zinc-200 font-mono text-sm leading-relaxed focus:outline-none resize-none bg-[#09090B] overflow-y-auto",
                theme === 'serif' && 'font-serif', theme === 'mono' && 'font-mono text-xs')} />
          </div>
        )}
        {(viewMode === 'preview' || viewMode === 'split') && (
          <div ref={previewRef} onScroll={handleScroll}
            className={cn("h-full overflow-y-auto px-6 py-6 bg-[#09090B] min-w-0 flex-1 prose prose-invert max-w-none",
              viewMode === 'preview' ? "w-full" : "w-1/2")}>
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-[#27272A] text-zinc-500 font-mono text-[10px] uppercase tracking-wider sticky top-0 bg-[#09090B] z-10">
              <span className="flex items-center gap-1"><BookOpen className="w-3 h-3 text-zinc-400" /> Live Preview</span>
            </div>
            {previewHtml ? (
              <div className="prose prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: previewHtml }} />
            ) : (
              <div className="h-[90%] flex flex-col items-center justify-center text-zinc-600 italic">预览内容为空</div>
            )}
          </div>
        )}
      </div>
