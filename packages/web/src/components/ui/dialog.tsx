...
function Dialog({ ignoreBack = false, ...props }: DialogRootProps) {
  ...
  React.useEffect(() => {
    ...
    registerOverlay(overlayId, () => onOpenChangeRef.current?.(false, { reason: undefined } as never));
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          "fixed top-[calc(var(--app-top-inset)+(var(--app-content-height)/2))] left-1/2 z-50 grid w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 overflow-y-auto rounded-xl bg-popover p-4 text-sm text-popover-foreground ring-1 ring-foreground/10 duration-100 outline-none sm:max-w-sm data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
          className
        )}
        style={{
          maxHeight: "calc(var(--app-content-height) - 2rem)",
          ...style,
        }}
        {...props}
      >
