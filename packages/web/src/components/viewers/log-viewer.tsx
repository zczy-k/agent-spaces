...
function LogViewerTerminal({
	...
	const t = useTranslations("terminal.logViewer")
		...
		>
			...
			<div className="flex shrink-0 items-center gap-2 border-b border-border/40 bg-muted/30 px-3 py-2">
						...
						{allLevels.map((level) => {
							...
							return (
								<button
									...
									aria-label={`${isActive ? t("hideLevel") : t("showLevel")} ${level} ${t("levelLogs")}`}
									className={cn(
				...
				<span className="mr-1 text-[10px] tabular-nums text-muted-foreground">
					...
					{searchQuery && ` / ${entries.length}`} {t("lines")}
				</span>
				...
				<div className="flex items-center gap-0.5">
					...
					<ToolbarButton
						...
						label={paused ? t("resumeAutoScroll") : t("pauseAutoScroll")}
						active={paused}
					>
						{paused ? <Play className="size-3.5" /> : <Pause className="size-3.5" />}
					</ToolbarButton>

					<ToolbarButton onClick={handleCopyAll} label={copied ? t("copied") : t("copyAllLogs")}>
						{copied ? <Check className="size-3.5 text-emerald-500" /> : <Copy className="size-3.5" />}
					</ToolbarButton>

					<ToolbarButton onClick={() => exportLogs(entries)} label={t("downloadLogs")}>
						<Download className="size-3.5" />
					</ToolbarButton>

					{onClear && (
						<ToolbarButton onClick={onClear} label={t("clearLogs")}>
			...
			>
				{filteredEntries.length === 0 ? (
					<Empty className="py-10">
						...
						<EmptyTitle>{searchQuery ? t("noMatchingLogEntries") : t("noLogEntries")}</EmptyTitle>
						<EmptyDescription>{searchQuery ? t("tryAdjustSearch") : t("logsWillAppear")}</EmptyDescription>
					</Empty>
				) : (
	...
	return (
		...
		>
			<div
				...
				role="log"
				aria-live="polite"
				aria-label={t("logOutput")}
			>
				{entries.length === 0 ? (
					<Empty className="py-8">
						<EmptyMedia variant="icon"><Terminal className="size-4" /></EmptyMedia>
						<EmptyTitle>{t("noLogEntries")}</EmptyTitle>
						<EmptyDescription>{t("logsWillAppear")}</EmptyDescription>
					</Empty>
				) : (
					entries.map((entry, i) => {
						const colors = resolveLevelColors(entry.level, colorScale)
		...
		>
			...
			<div className="flex items-center gap-2 border-b border-border/40 bg-muted/30 px-3 py-2">
				...
				<div className="flex items-center gap-0.5">
					<ToolbarButton
						...
						label={copied ? t("copied") : t("copyFilteredLogs")}
					>
						{copied ? <Check className="size-3.5 text-emerald-500" /> : <Copy className="size-3.5" />}
					</ToolbarButton>

					<ToolbarButton onClick={() => exportLogs(filteredEntries)} label={t("downloadLogs")}>
						<Download className="size-3.5" />
					</ToolbarButton>
			...
			>
				{filteredEntries.length === 0 ? (
					<Empty className="py-10">
						<EmptyMedia variant="icon"><Terminal className="size-4" /></EmptyMedia>
						<EmptyTitle>{t("noMatchingLogEntries")}</EmptyTitle>
						<EmptyDescription>
