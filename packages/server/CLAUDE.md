...
- **启动流程**：Express 初始化 -> auth 中间件注册 -> 路由注册（含 workflow/command/search/subscription/speech-recognition/skill/mcp/notification） -> HTTP Server 创建 -> WebSocket Server 创建（含 /ws 和 /ws/speech） -> 启动 Issue 重试恢复 -> 启动持久化通知服务

## 对外接口

### REST API 路由表

所有路由挂载在 `/api/` 下，除 `/api/health`、`/api/auth/login`、`/api/auth/check` 外均需 Bearer Token 认证。Agent SSE API 支持 Bearer Token / x-agent-spaces-key Header / key Body 三种认证方式。
...
| `/api/user/settings` | GET/PUT | 用户设置（avatarUrl）（**新**） |
...
| `/api/workspaces/:id/notifications` | GET | 列出应用内通知（**新**） |
| `/api/workspaces/:id/notifications/:notificationId/read` | PUT | 标记通知已读（**新**） |
| `/api/workspaces/:id/notifications/read-all` | PUT | 全部标记已读（**新**） |
| `/api/workspaces/:id/notifications` | DELETE | 清空所有通知（**新**） |
| `/api/workspaces/:id/commands` | GET/POST | 列出/创建快捷命令（**新**） |
| `/api/workspaces/:id/commands/processes` | GET | 列出运行中命令进程（**新**） |
| `/api/workspaces/:id/commands/:commandId` | PUT/DELETE | 更新/删除快捷命令（**新**） |
| `/api/workspaces/:id/commands/:commandId/run` | POST | 运行快捷命令（**新**） |
| `/api/workspaces/:id/commands/:commandId/stop` | POST | 停止快捷命令（**新**） |
| `/api/workspaces/:id/search/code` | GET | 代码搜索（ripgrep/Node.js）（**新**） |
| `/api/workspaces/:id/search/files` | GET | 文件名搜索（**新**） |
...
| `/api/agent-sse/run` | POST | Agent SSE 流式调用（**新**） |
...
| `/api/skills` | GET | 列出全局技能（**新**） |
| `/api/skills/sync-check` | GET | 检查技能同步状态（**新**） |
| `/api/skills/sync` | POST | 同步技能（**新**） |
| `/api/skills/import` | POST | 导入技能文件（**新**） |
| `/api/skills/:name` | PUT/DELETE | 更新/删除技能（**新**） |
| `/api/mcps` | GET | 列出全局 MCP 配置（**新**） |
| `/api/mcps/import` | POST | 导入 MCP 配置（**新**） |
| `/api/mcps/:name` | PUT/DELETE | 更新/删除 MCP 配置（**新**） |
| `/api/subscriptions` | GET/POST | 列出/创建订阅配置（**新**） |
| `/api/subscriptions/:id` | PUT/DELETE | 更新/删除订阅配置（**新**） |
| `/api/subscriptions/:id/quota` | GET | 查询订阅配额（**新**） |
| `/api/speech-recognition` | GET/POST | 列出/创建语音识别配置（**新**） |
| `/api/speech-recognition/:id` | PUT/DELETE | 更新/删除语音识别配置（**新**） |
| `/api/git-config` | GET/POST | 全局 Git 配置（**新**） |
...
| `ws://localhost:3100/ws/speech?token=<token>&configId=<id>` | token 查询参数 | 语音识别流式 WebSocket（**新**） |
...
- `command.started` / `command.stopped` / `command.restarted`（**新**）
- `notification.created` / `notification.cleared`（**新**）
...
| `LangChainRuntime` | `langchain`（**新**） | `langchain` + `@langchain/openai` + `@langchain/anthropic` + `@langchain/google-genai` | 基于 LangChain.js createAgent API，provider-neutral |
...
HTTP Server-Sent Events 流式 Agent 调用端点（**新**）。
...
基于 ripgrep（优先）+ Node.js（回退）的代码搜索（**新**）。
...
`services/notification-center.ts` 实现应用内通知 CRUD 和 WebSocket 推送（**新**）：
...
| `langchain` + `@langchain/openai` + `@langchain/anthropic` + `@langchain/google-genai` | LangChain Agent 运行时（**新**） |
...
~/.agent-spaces-data/
  ...
  subscriptions.json                # 订阅配置列表（新增）
  speech-recognition.json           # 语音识别配置列表（新增）
  user-settings.json                # 用户设置（avatarUrl）（新增）
```
...
```
packages/server/src/
  app.ts                          # Express 入口，路由注册（含 workflow/command/search/subscription/speech/skill/mcp/notification），WebSocket 服务（含 /ws + /ws/speech），头像上传，认证中间件，用户设置，全局 Git 配置，启动恢复
  middleware/
  ...
  routes/
    ...
    agent-sse.ts                  # Agent SSE 流式调用（新增）
    workflow.ts                   # Workflow CRUD + duplicate 路由
    command.ts                    # 快捷命令 CRUD + run/stop/processes（新增）
    search.ts                     # 代码搜索 + 文件名搜索（新增）
    subscription.ts               # 订阅配置 CRUD + quota 查询（新增）
    speech-recognition.ts         # 语音识别配置 CRUD + WebSocket handler（新增）
    skill.ts                      # 全局技能管理（新增）
    mcp.ts                        # 全局 MCP 配置管理（新增）
    notification.ts               # 应用内通知 CRUD（新增）
  ...
  services/
    ...
    pty.ts                        # PTY 终端会话管理
    search.ts                     # 代码搜索服务（ripgrep + Node.js + 文件名搜索）（新增）
    gitignore.ts                  # .gitignore 解析过滤器（新增）
    command.ts                    # 快捷命令 CRUD 服务（新增）
    command-process-manager.ts    # 快捷命令进程生命周期管理（新增）
    skill.ts                      # 全局技能管理服务（新增）
    mcp.ts                        # 全局 MCP 配置管理服务（新增）
    notification-center.ts        # 应用内通知 CRUD + WebSocket 推送（新增）
    subscription/                 # 订阅管理子目录（新增）
  ...
  storage/
    ...
    command-store.ts              # 快捷命令持久化（新增）
    subscription-store.ts         # 订阅配置持久化（新增）
    speech-recognition-store.ts   # 语音识别配置持久化（新增）
    user-settings-store.ts        # 用户设置持久化（新增）
  ...
  adapters/
    ...
    langchain-runtime.ts          # LangChain.js 运行时实现（新增）
  ...
  agents/
    ...
    agent-designer.ts             # AI Agent 预设生成器（新增）
  ws/
    handler.ts                    # WebSocket 事件路由中心（@mention + 实时消息）
    agent-runner.ts               # @mention Agent 运行器（634 行，从 handler 提取）（新增）
    message-parts.ts              # 消息 Parts 构建管线（523 行，从 agent-message-parts 提取）（新增）
    agent-prompt.ts               # Agent Prompt 构建（从 handler 提取）（新增）
    terminal-handler.ts           # 终端事件处理
    connection-manager.ts         # WebSocket 连接管理 + 广播
    html-utils.ts                 # HTML 文本清理工具（新增）
  hooks/
