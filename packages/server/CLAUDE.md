...
- **启动流程**：Express 初始化 -> auth 中间件注册 -> 路由注册（含 workflow/command/search/subscription/speech-recognition/skill/mcp/notification/code-favorites/prompt-template/hooks/output-style） -> Inspector track 端点（免认证） -> HTTP Server 创建 -> WebSocket Server 创建（含 /ws + /ws/speech + /ws/lsp/typescript） -> 启动 Issue 重试恢复 -> 启动持久化通知服务
...
| `/api/workspaces/:id/hooks` | GET/POST | 列出/创建 Hook 配置（**新**） |
| `/api/workspaces/:id/hooks/:name` | GET/PUT/DELETE | 获取/更新/删除 Hook 配置（**新**） |
| `/api/workspaces/:id/hooks/upload` | POST | 上传 Hook JSON（**新**） |
| `/api/workspaces/:id/hooks/:name/apply` | POST | 应用 Hook 到另一个工作空间（**新**） |
...
| `/api/output-styles` | GET/POST | 列出/创建输出风格模板（**新**） |
| `/api/output-styles/:id` | PUT/DELETE | 更新/删除输出风格模板（**新**） |
...
Agent 工具调用前后的自定义钩子系统，per-tool-call 粒度（**新**）。
...
`agents/issue-task-controller.ts` 是从 issue-agent-runner 独立拆分出的任务控制器（689 行）（**新**）。
...
Issue Task Controller（issue-task-controller.ts，689 行）:
...
```
packages/server/src/
  app.ts                          # Express 入口，路由注册（含 code-favorites/prompt-template/hooks/output-style/inspector-track），WebSocket 服务（含 /ws + /ws/speech + /ws/lsp/typescript），头像上传，认证中间件，用户设置，全局 Git 配置，启动恢复
  middleware/
  ...
  routes/
    ...
    hooks.ts                      # Hook 配置 CRUD + upload + apply（新增）
    output-style.ts               # 输出风格模板 CRUD（新增）
  ...
  services/
    ...
    hook-engine.ts                # Hook 引擎（规则匹配/命令执行/wrapOnEventWithHooks）（新增）
    output-style.ts               # 输出风格模板管理 + resolveOutputStyleContent（新增）
    ...
    notification-hub/             # 通知中心子目录（14 文件）
      ...
      bot-agent.ts                # Bot Agent 执行与上下文构建（新增，从 notification-hub 提取）
  ...
  adapters/
    ...
    agent-runtime-types.ts        # Agent 运行时接口定义（AgentRuntime, AgentRunResult, AgentRuntimeEvent, AgentFunctionTool）（新增，从 agent-runtime 拆分）
  ...
  agents/
    ...
    issue-task-controller.ts      # 任务控制器（Task Creator + Workflow Task 创建 + 依赖调度 + Executor 启动）（新增，从 issue-agent-runner 拆分）
...
| 时间 | 操作 | 说明 |
|------|------|------|
| 2026-05-20T14:08:52+08:00 | 增量更新 | **Hook 系统**（新增 routes/hooks.ts + services/hook-engine.ts + storage/hook-store.ts，CRUD + wrapOnEventWithHooks 拦截 AgentRuntimeEvent，支持 PreToolUse/PostToolUse、shell/webhook/script 三种动作，.hook.json 文件持久化）；**输出风格管理**（新增 routes/output-style.ts + services/output-style.ts，OutputStyleTemplate CRUD + resolveOutputStyleContent 运行时注入，meta.json 持久化）；**Issue Task Controller 拆分**（新增 agents/issue-task-controller.ts，从 issue-agent-runner 独立拆分的 689 行任务控制器，含 Task Creator + 依赖调度 + Hook 集成）；**Agent 运行时接口提取**（新增 adapters/agent-runtime-types.ts，AgentRuntime/AgentRunResult/AgentRuntimeEvent/AgentFunctionTool 独立文件）；**Bot Agent 提取**（新增 services/notification-hub/bot-agent.ts，Bot Agent 执行与上下文构建从 notification-hub 提取）；**API 路由新增** /api/workspaces/:id/hooks（CRUD + upload + apply）、/api/output-styles（CRUD）；**数据持久化新增** hooks/ 目录（.hook.json）、output-styles/meta.json；**notification-hub 文件数 12->14**；**server 版本 0.3.61->0.3.62**；**文件数 113->118** |
...
| 2026-05-16T17:36:40+08:00 | 增量更新 | **第四运行时 LangChain**（新增 adapters/langchain-runtime.ts）；**订阅管理**（新增 services/subscription/ 5 文件 + storage/subscription-store.ts + routes/subscription.ts）；**语音识别**（新增 services/speech-recognition/ 3 文件 + storage/speech-recognition-store.ts + routes/speech-recognition.ts + /ws/speech）；**快捷命令**（新增 services/command.ts + services/command-process-manager.ts + storage/command-store.ts + routes/command.ts）；**代码搜索**（新增 services/search.ts + services/gitignore.ts + routes/search.ts）；**Agent SSE API**（新增 routes/agent-sse.ts）；**Agent Designer**（新增 agents/agent-designer.ts）；**应用内通知**（新增 services/notification-center.ts + routes/notification.ts）；**Skill/MCP 管理**（新增 services/skill.ts + services/mcp.ts + routes/skill.ts + routes/mcp.ts）；**WS 重构**（handler.ts 提取到 ws/agent-runner.ts + ws/message-parts.ts + ws/agent-prompt.ts）；**server 版本 0.2.4->0.3.0**；**文件数 73->106** |
| 2026-05-08T17:18:31+08:00 | 增量更新 | **Workflow 系统**（新增 routes/workflow.ts + services/workflow.ts + storage/workflow-store.ts）；**文件数 70->73** |
