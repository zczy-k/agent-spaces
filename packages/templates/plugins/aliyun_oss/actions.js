...
function createConfigProperties(t) {
  return [
    { key: 'region', label: 'Region', type: 'text', required: true, toolRequired: false, tooltip: t('config.region.tooltip', 'Example: oss-cn-hangzhou'), default: `${CONFIG_PREFIX}["region"]}}` },
    ...
    { key: 'endpoint', label: 'Endpoint', type: 'text', tooltip: t('config.endpoint.tooltip', 'Custom endpoint. Region is ignored when endpoint is provided.'), default: `${CONFIG_PREFIX}["endpoint"]}}` },
  ...
  return [
    {
      ...
      run: async (ctx, args) => {
        ...
        let result
        if (args.filePath) {
          ctx.logger.info(`Uploading file: ${args.filePath} -> ${args.objectKey}`)
          result = await client.put(args.objectKey, args.filePath)
        } else if (args.content) {
          ctx.logger.info(`Uploading content: -> ${args.objectKey}`)
    ...
    },
    {
      ...
      run: async (ctx, args) => {
        ...
        const client = createClient(args)
        if (args.filePath) {
          ctx.logger.info(`Downloading file: ${args.objectKey} -> ${args.filePath}`)
        ...
        ctx.logger.info(`Reading content: ${args.objectKey}`)
    ...
    },
    {
      ...
      run: async (ctx, args) => {
        ...
        if (Array.isArray(args.objectKeys) && args.objectKeys.length) {
          ctx.logger.info(`Deleting multiple files: ${args.objectKeys.length} files`)
        ...
        ctx.logger.info(`Deleting file: ${args.objectKey}`)
    ...
    },
    {
      ...
      tool: false,
      properties: [
        { key: 'objectKeys', label: t('field.objectKeys.label', 'Object Key List'), type: 'textarea', required: true, tooltip: t('field.objectKeysJson.tooltip', 'JSON array, for example ["a.txt","b.txt"].') },
      ],
      configProperties,
      outputs: [
        ...commonOutputs,
      ...
      run: async (ctx, args) => {
        ...
        ctx.logger.info(`Deleting multiple files: ${keys.length} files`)
        const result = await client.deleteMulti(keys)
        return {
          success: true,
          message: t('message.batchDeleteCount', 'Batch delete completed. Deleted {count} files.').replace('{count}', String(result.deleted?.length || 0)),
    ...
    },
    {
      ...
      properties: [
        ...
        { key: 'maxKeys', label: t('field.maxKeys.label', 'Max Keys'), type: 'number', default: 100, tooltip: t('field.maxKeys.tooltip', 'Maximum number of results returned in one request.') },
      ...
      run: async (ctx, args) => {
        ...
        ctx.logger.info(`Listing files: prefix=${args.prefix || '(all)'}, max=${args.maxKeys || 100}`)
    ...
    },
    {
      ...
      properties: [
        ...
        { key: 'responseContentType', label: 'Response Content-Type', type: 'text', tooltip: 'Example: application/octet-stream' },
        { key: 'responseContentDisposition', label: 'Response Content-Disposition', type: 'text', tooltip: 'Example: attachment; filename="file.txt"' },
      ],
      configProperties,
      outputs: [
        ...commonOutputs,
      ...
      run: async (ctx, args) => {
        ...
        ctx.logger.info(`Generating signed URL: ${args.objectKey}, expires=${args.expires || 3600}s`)
    ...
    },
    {
      ...
      run: async (ctx, args) => {
        ...
        ctx.logger.info(`Copying file: ${source} -> ${args.objectKey}`)
        const result = await client.copy(args.objectKey, source)
        const out = normalizeResult(result)
        return {
          success: true,
          message: t('message.copySuccess', 'Copy succeeded: {sourceKey} -> {objectKey}')
            .replace('{sourceKey}', args.sourceKey)
            .replace('{objectKey}', args.objectKey),
          data: out,
        }
      },
    },
