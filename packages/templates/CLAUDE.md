...
Agent 预设模板库。提供 376 个模板文件，覆盖 Agent 预设（184 个）、MCP 服务器配置（9 个）、Skills（15 个）、Workflow Plugins（107 个）、Workflow 模板、Prompt 模板、Output Style 模板等多种类型。通过 `generate-index.mjs` 自动生成索引文件，支持 Store 在线导入。
...
- **分类体系**：Agent 按功能域分为 15 个分类，每个分类独立目录
- **Store 集成**：通过 HTTP 静态服务暴露给 Agent Store 在线导入
- **多模板类型**：统一管理 Agent/MCP/Skill/Plugin/Workflow/Prompt/OutputStyle 七种模板
