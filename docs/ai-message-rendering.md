...
`runMentionedAgent()` 当前会从当前频道读取最近 20 条消息作为历史上下文：

```ts
listMessages(workspaceId, channelId, { limit: 20 })
```

历史上下文不是 runtime 的完整结构化 transcript。它不会传入 Claude/Codex/OpenAgent SDK 原始 tool call 或 tool result 消息，也不会传入工具详情懒加载里的 input/output。Prompt 中的 `Conversation history` 只包含：

- 用户消息：`Message.content` 去 HTML 后的文本。
- agent 消息：每条 agent 消息的 final message，也就是结构化 `parts` 里的 `type: "text"` 内容。

如果 agent 消息已有结构化 `parts` 但没有 final `text` part，通常表示还在运行、等待用户或旧数据不完整，这条 agent 历史会被跳过。对没有 `parts` 的旧 completed agent 消息，会从 `content` 做兼容兜底，并过滤常见工具调用展示行。
