...
- **前端 SDK**：@agent-spaces/sdk（37 模块的统一 API 调用层，HttpClient 封装 + Bearer Token 自动注入 + 调试日志）
...
- **SDK 层**：前端 API 统一调用包（@agent-spaces/sdk），按模块拆分 37 个 API 适配器，web 通过 sdk.ts 单例消费
- **模板层**：Agent 预设模板包（@agent-spaces/agents），含 100+ 预制 Agent 模板（academic/design/engineering/finance/game-development/marketing 等分类），支持 Store 在线导入
...
| templates | `packages/templates` | JSON/Markdown | 376 | Agent 预设模板库（@agent-spaces/agents），含 184 Agent 预设（15 分类）+ 9 MCP + 15 Skill + 107 Plugin + 1 Workflow + Prompt + Output Style 模板，支持 generate-index 自动索引 + http-server 静态托管 + Store 在线导入 |
...
36. Workflow 高级编辑器：支持分组/循环节点、属性面板、变量选择器、版本管理、暂存面板、操作历史、执行栏、触发器配置（cron/webhook）、嵌入编辑器、插件系统（自定义节点 + 配置 + Store 安装）
...
- **Workflow 执行引擎测试**：DAG 遍历/循环/分支/变量/断点/恢复（execution-manager 1393 行核心引擎）
...
- DOM Inspector `/api/inspector/track` 兆认证（被调试项目调用）
...
- 数据持久化使用 JSON 文件（Workspace/Issue/Task/Channel/Message/LLM/Workflow/Command/Subscription/SpeechConfig/Notification/CodeFavorites/PromptTemplates/Hooks/OutputStyles/RobotAccounts/Worktrees/PluginState）+ SQLite（Agent Session/Usage + Kanban Board + DocNode Database）
