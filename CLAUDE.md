...
- **Agent 运行时**：支持五种运行时 -- `OpenAgentSdkRuntime`（基于 @codeany/open-agent-sdk）、`ClaudeCodeRuntime`（基于 @anthropic-ai/claude-agent-sdk，已拆分为 7 文件子模块）、`CodexRuntime`（基于 @openai/codex-sdk + CodexFunctionToolBridge MCP 桥接）、`LangChainRuntime`（基于 langchain）、`HermesRuntime`（外部 Hermes CLI 进程适配），通过工厂函数 `createAgentRuntime()` 按配置切换
...
- **i18n 国际化**：next-intl + LocaleProvider，中英文切换，25+ 命名空间按功能拆分（agent/chat/commands/composer/database/editor/git/home/issue/kanban/sidebar/terminal/workspace 等各独立 JSON 文件），52+ 组件已完成改造
...
- i18n 使用 next-intl，翻译文件按命名空间拆分为 `src/locales/{en,zh}/*.json`（25+ 命名空间），组件通过 `useTranslations('namespace')` 获取
