...
- **前端 SDK**：@agent-spaces/sdk（38 模块的统一 API 调用层，HttpClient 封装 + Bearer Token 自动注入 + 调试日志）
...
- **SDK 层**：前端 API 统一调用包（@agent-spaces/sdk），按模块拆分 38 个 API 适配器，web 通过 sdk.ts 单例消费
