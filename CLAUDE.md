...
4. 创建 Workflow 模板（可视化 DAG 编辑器，支持 Agent/Command/Group/Loop 节点，连线定义依赖，版本管理，触发器配置）或使用已有模板
...
36. Workflow 高级编辑器：支持分组/循环节点、属性面板、变量选择器、版本管理、暂存面板、操作历史、执行栏、触发器配置（cron/webhook）、嵌入编辑器
...
- **Workflow 执行引擎测试**：DAG 遍历/循环/分支/变量/断点/恢复（execution-manager 1321 行核心引擎）
...
- Workflow 使用 WorkFox 引擎驱动的 DAG 拓扑（@xyflow/react 前端 + execution-manager 后端执行引擎），支持 Agent/Command/Group/Loop 节点，含触发器（cron/webhook）、版本管理、断点调试、交互管理、变量系统
